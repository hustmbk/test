...
def apply_decoupled_rope(query_states, key_states, position_ids, rope_dim, cos_cache, sin_cache):
    ...
    if sin_cache is None:
        # 如果sin_cache为None，说明cos_cache已经是合并格式
        cos_sin_cache = cos_cache
    elif cos_cache.dim() == 2 and sin_cache.dim() == 2:
        # 如果是分离的cos和sin缓存，需要合并
        cos_sin_cache = torch.cat([cos_cache, sin_cache], dim=-1)
    else:
        # 如果已经是合并格式
        cos_sin_cache = cos_cache
    
    # 确保在正确设备上，并使用与query/key张量相同的数据类型（FlashInfer要求fp16/bf16）
    cos_sin_cache = cos_sin_cache.to(device=q_rope.device, dtype=q_rope.dtype)
    
    # FlashInfer RoPE API需要正确的position_ids格式
    # 将二维position_ids转换为一维格式，这是FlashInfer期望的格式
    position_ids_flat = position_ids.reshape(-1)
    
    flashinfer.rope.apply_rope_with_cos_sin_cache_inplace(
        position_ids_flat,  # 使用一维position_ids格式
        q_rope,
        k_rope,
        rope_dim,  # 对于解耦RoPE，这里应该是rope_dim而不是完整的head_dim
        cos_sin_cache,  # 传递完整缓存，应该是float32
        True  # interleaved
    )
